...
class IndicTransLoader(BartLoader):
    ...
    def get_model_spec(self, model):
        ...
        # IndicTrans2 DOES apply a LayerNorm to the summed embeddings+PE
        # (modeling: self.layernorm_embedding). ctranslate2's BART loader reads this via
        # model.config.normalize_embedding (IndicTransConfig has no such key, so default True).
        # We MUST keep it True so the real layernorm_embedding weights are serialized;
        # setting it False drops the embedding LayerNorm and scrambles all outputs (Odia drift).
        model.config.normalize_embedding = True
        if os.environ.get("CT2_NO_SCALE") == "1":
            model.model.encoder.embed_scale = 1.0
            model.model.decoder.embed_scale = 1.0
        return super().get_model_spec(model)

    def set_position_encodings(self, spec, module):
        # IndicTransSinusoidalPositionalEmbedding stores a 'weights' buffer whose first
        # `offset` rows are padding; the effective PE at decode position p is weights[offset + p].
        # So hand ctranslate2 weights[offset : offset + max_pos].
        w = module.weights
        off = int(getattr(module, "offset", 0) or 0)
        max_pos = w.shape[0] - off
        # Allow overriding the offset via env for experiments (CT2_PE_OFFSET).
        off = int(os.environ.get("CT2_PE_OFFSET", str(off)))
        max_pos = w.shape[0] - off
        spec.encodings = w.numpy()[off: off + max_pos]
...
def _prune_vocab(model, tokenizer):
    ...
    def keep(tok):
        if tok in SPECIAL:
            return True
        t = tok.replace("\u2581", "")  # strip sentencepiece space marker
        if t == "":
            return True
        for ch in t:
            o = ord(ch)
            if 0x0900 <= o <= 0x097F:  # Devanagari
                return True
            if 0x1C50 <= o <= 0x1C7F:  # Ol Chiki
                return True
            if 0x0041 <= o <= 0x024F:  # Latin
                return True
            if 0x0030 <= o <= 0x0039:  # ASCII digits
                return True
        return False
...
def _patched_call(self, model, tokenizer):
    # ctranslate2 BART will NOT tolerate a reindexed vocabulary (even an identity
    # reindex collapses decoding -> "eng_Latn"). So pruning via reindex is OFF by
    # default. The working baseline keeps the original vocab order; Odia is instead
    # suppressed at inference with suppress_sequences. Set CT2_PRUNE=1 only if you
    # have verified a reindex-safe path.
