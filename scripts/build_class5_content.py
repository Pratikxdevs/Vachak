"""Build Class 5 content pack (text + assignments + worksheets + flashcards).

Class-5 SPECIAL script (not generalized). Reads the Santali PDFs under
santali_organized/Class 5 (Arts 1 + General 1, both true Class-5, plus 3
Class-4 PE chapters unique to this folder, flagged grade=4) and emits
deterministic, review-gated content under curriculum/class/5/:
...
Rules (per AGENTS.md):
  ...
  * The 4 Class-4 PE chapters byte-identical to the Class-3 folder copies are
    NOT re-packed here; they already live in curriculum/class/3 (see SKIPPED).
    The misfiled Class-1 EVS book is packed in curriculum/class/1.

Usage:
    python scripts/build_class5_content.py
"""
...
C5 = os.path.join(ROOT, "santali_organized", "Class 5")

# (relative path under santali_organized/Class 5, chapter slug, subject, grade)
CLASS5_BOOKS = [
    # --- Arts (true Class 5) ---
    ("Arts/books/Class_5__1__Objects_on_the_Move__Santhali_.pdf", "objects-on-the-move", "Arts", 5),
    # --- General (true Class 5) ---
    ("General/books/Class_5__11__Music_Around_Me__Santhali_.pdf", "music-around-me", "General", 5),
    # --- Class-4 PE chapters unique to this folder (flagged grade=4) ---
    ("Physical Education  & Well Being/assignments/Class_4__Chapter_-_3_Strike_the__Shuttlecock__Santhali_.pdf", "strike-the-shuttlecock", "Physical Education & Well Being", 4),
    ("Physical Education  & Well Being/assignments/Class_4__Chapter_-_4_Little_Steps__Santhali_.pdf", "little-steps-g4", "Physical Education & Well Being", 4),
    ("Physical Education  & Well Being/assignments/Class_4__Chapter_-_7_Yoga_Sadhana__Santhali_.pdf", "yoga-sadhana", "Physical Education & Well Being", 4),
]

# Files deliberately NOT packed here (already packed elsewhere, by md5):
SKIPPED = [
    ("Physical Education  & Well Being/assignments/Class_4__Chapter_-_1_Throwing_and_Catching__Santhali_.pdf", "byte-identical copy already packed in curriculum/class/3"),
    ("Physical Education  & Well Being/assignments/Class_4__Chapter_-_2_Kicking_and__Receiving__Santhali_.pdf", "byte-identical copy already packed in curriculum/class/3"),
    ("Physical Education  & Well Being/assignments/Class_4__Chapter_-_5_Local_and_Traditional_Games__Santhali_.pdf", "byte-identical copy already packed in curriculum/class/3"),
    ("Physical Education  & Well Being/assignments/Class_4__Chapter_-_6_Yoga_for_Daily__Life__Santhali_.pdf", "byte-identical copy already packed in curriculum/class/3"),
    ("EVS/books/Class_1__Chapter_10_How_do_I_Spend_my_Day___Time___Santhali_.pdf", "already packed in curriculum/class/1"),
]
...
def build_book(pdf_path: str, slug: str, subject: str, grade: int, out_ch: str,
    ...
    chapter = {
        ...
        "source_pdf": os.path.relpath(pdf_path, C5), "pages": len(doc),
        "chars": len(full_text),
...
def build_worksheet(chapter: dict, assigns: list[dict], vocab: list[str]) -> dict:
    ...
    return {
        ...
        "meta": {"grade": grade, "subject": chapter["subject"], "chapter": cid,
                 ...
                 "generator": "scripts/build_class5_content.py (deterministic, offline)",
...
def main() -> int:
    ...
    ap.add_argument("--src", default=C5)
    ap.add_argument("--out", default=os.path.join(ROOT, "curriculum", "class", "5"))
    args = ap.parse_args()
    ...
    seen: dict = {}
    stats: dict = {}
    review_lines = ["# Class 5 — Speaker Review Sheet", "",
                    "Status: AUTO_EXTRACTED. Approve or correct per chapter.",
                    "Note: 3 Class-4 PE chapters unique to this folder are packed with",
                    "grade=4 + grade_mismatch. 4 byte-identical Class-4 PE copies were",
                    "skipped (already in curriculum/class/3); the Class-1 EVS book went",
                    "to curriculum/class/1.", ""]
    for rel, slug, subject, grade in CLASS5_BOOKS:
    ...
    for rel, reason in SKIPPED:
        review_lines += [f"## SKIPPED (already packed elsewhere): {rel}",
                         f"- {reason}", ""]
    manifest = {"grade": 5, "books": len(stats),
    ...
    # Full fidelity: pre-trim total ~=9MB, so no budget trim for Class 5.
