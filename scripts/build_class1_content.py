"""Build Class 1 content pack (text + assignments + worksheets + flashcards).

Class-1 SPECIAL script (not generalized). Reads the 6 Class-1 Santali PDFs
scattered across santali_organized (Primer + 3 Mathematics under Class 1-2,
1 Mathematics under Class 4, 1 EVS under Class 5) and emits deterministic,
review-gated content under curriculum/class/1/:
...
Rules (per AGENTS.md):
  ...
  * The 8 Class-2 PDFs sitting in the Class 1-2 folder are NOT packed here;
    they already live in curriculum/class/2 (see SKIPPED).

Usage:
    python scripts/build_class1_content.py
"""
...
C1 = os.path.join(ROOT, "santali_organized")

# (relative path under santali_organized, chapter slug, subject, grade)
CLASS1_BOOKS = [
    # --- General: Santali-Hindi primer ---
    ("Class 1-2/General/books/Class_1__Santali__Hindi__Primer.pdf", "santali-hindi-primer", "General", 1),
    # --- Mathematics (Class 1-2 folder) ---
    ("Class 1-2/Mathematics/assignments/Class_1__Chapter_13_So_Many_Toys__Data_Handling___Santhali_.pdf", "so-many-toys", "Mathematics", 1),
    ("Class 1-2/Mathematics/assignments/Class_1__Chapter_8_Fun_with_Numbers__Numbers_21_to_99___Santhali_.pdf", "fun-with-numbers-21-99", "Mathematics", 1),
    ("Class 1-2/Mathematics/books/Class_1__Chapter_1_Finding_the_Furry_Cat___Pre-number_Concepts___Sant.pdf", "finding-the-furry-cat", "Mathematics", 1),
    # --- Mathematics misfiled under Class 4 ---
    ("Class 4/Physical Education  & Well Being/books/Class_1__Chapter_11_How_Many_Times___Multiplication___Santhali_.pdf", "how-many-times", "Mathematics", 1),
    # --- EVS misfiled under Class 5 ---
    ("Class 5/EVS/books/Class_1__Chapter_10_How_do_I_Spend_my_Day___Time___Santhali_.pdf", "how-do-i-spend-my-day", "Environmental Studies", 1),
]

# Files deliberately NOT packed here (already packed elsewhere, by md5):
SKIPPED = [
    ("Class 1-2/Mathematics/assignments/Class_2__Decoration_for_Festival__Santhali_.pdf", "already packed in curriculum/class/2"),
    ("Class 1-2/Mathematics/assignments/Class_2__Fun_at_the_Fair__Santhali_.pdf", "already packed in curriculum/class/2"),
    ("Class 1-2/Mathematics/assignments/Class_2__Grouping_and_Sharing__Santhali_.pdf", "already packed in curriculum/class/2"),
    ("Class 1-2/Mathematics/assignments/Class_2__Playing_with_Lines__Santhali_.pdf", "already packed in curriculum/class/2"),
    ("Class 1-2/Mathematics/assignments/Class_2__Rani_s_Gift__Santhali_.pdf", "already packed in curriculum/class/2"),
    ("Class 1-2/Mathematics/assignments/Class_2__Shadow_Story__Togalu___Santhali_.pdf", "already packed in curriculum/class/2"),
    ("Class 1-2/Mathematics/assignments/Class_2__Shapes_Around_Us__Santhali_.pdf", "already packed in curriculum/class/2"),
    ("Class 1-2/Mathematics/assignments/Class_2__Which_Season_is_it___Santhali_.pdf", "already packed in curriculum/class/2"),
]
...
def build_book(pdf_path: str, slug: str, subject: str, grade: int, out_ch: str,
    ...
    chapter = {
        ...
        "source_pdf": os.path.relpath(pdf_path, C1), "pages": len(doc),
        "chars": len(full_text),
...
def build_worksheet(chapter: dict, assigns: list[dict], vocab: list[str]) -> dict:
    ...
    return {
        ...
        "meta": {"grade": grade, "subject": chapter["subject"], "chapter": cid,
                 ...
                 "generator": "scripts/build_class1_content.py (deterministic, offline)",
...
def main() -> int:
    ...
    ap.add_argument("--src", default=C1)
    ap.add_argument("--out", default=os.path.join(ROOT, "curriculum", "class", "1"))
    args = ap.parse_args()
    ...
    seen: dict = {}
    stats: dict = {}
    review_lines = ["# Class 1 — Speaker Review Sheet", "",
                    "Status: AUTO_EXTRACTED. Approve or correct per chapter.",
                    "Note: Class-1 books are scattered across folders; source_pdf records",
                    "the true location. 8 Class-2 PDFs in the Class 1-2 folder were skipped",
                    "(already packed in curriculum/class/2).", ""]
    for rel, slug, subject, grade in CLASS1_BOOKS:
    ...
    for rel, reason in SKIPPED:
        review_lines += [f"## SKIPPED (already packed elsewhere): {rel}",
                         f"- {reason}", ""]
    manifest = {"grade": 1, "books": len(stats),
    ...
    # Full fidelity: pre-trim total ~=9MB, so no budget trim for Class 1.
