...
class IndicTrans2Adapter(
        ...
        return try {
            ...
            for (name in mergedAssetFiles) {
                ...
                try {
                    ...
                    android.util.Log.d("Vachak-MT", "extracted merged asset $name (${dst.length() / 1024 / 1024}M)")
                } catch (e: Exception) {
                    android.util.Log.w("Vachak-MT", "merged asset missing $name: ${e.message}")
                    if (name == "model.bin") ok = false
                }
            }
            if (ok && modelOut.exists()) outDir.absolutePath else null
        } catch (e: Exception) {
            android.util.Log.e("Vachak-MT", "merged extraction failed", e)
            null
        }
    }
    ...
    private fun resolveModelPath(packId: String?): String {
        ...
        if (!packId.isNullOrBlank()) {
            val f = File(packId)
            if (f.exists()) {
                android.util.Log.d("Vachak-MT", "resolveModelPath packId hit $packId")
                return packId
            }
            ...
            if (filesDirPack.exists()) {
                android.util.Log.d("Vachak-MT", "resolveModelPath filesDir pack hit ${filesDirPack.absolutePath}")
        ...
        for (c in candidates) {
            if (c.exists()) {
                android.util.Log.d("Vachak-MT", "resolveModelPath filesDir hit ${c.parent}")
        ...
        android.util.Log.w("Vachak-MT", "resolveModelPath miss packId=$packId filesDir=$filesDir -> fallback $modelPath (pack not installed, using $modelPath or host refMap)")
        ...
        if (hostCandidate.exists()) {
            android.util.Log.d("Vachak-MT", "resolveModelPath host project root hit ${hostCandidate.absolutePath}")
    ...
    override fun supports(pair: LanguagePair): Boolean {
        ...
        if (!ok) android.util.Log.w("Vachak-MT", "supports false ${pair.source}->${pair.target} (src=$src tgt=$tgt)")
        return ok
    }
    ...
    override fun loadModel(packId: String): EngineResult<Unit> {
        return try {
            ...
            android.util.Log.d("Vachak-MT", "loadModel packId=$packId resolved=$path nativeAvailable=$nativeAvail handle=$handle")
            ...
            if (nativeAvail && !modelExists) {
                ...
                android.util.Log.e("Vachak-MT", "CT2 model not found at $path/model.bin - pack not installed, using fallback Err")
                ...
                android.util.Log.w("Vachak-MT", "Model missing, using mock handle to keep app alive (install pack to fix)")
                synchronized(lock) {
            ...
            synchronized(lock) {
                if (handle != 0L && lastModelPath == path) {
                    android.util.Log.d("Vachak-MT", "loadModel cached handle=$handle path=$path")
                ...
                handle = if (nativeAvail) {
                    android.util.Log.d("Vachak-Native", "Ct2Jni.init $path")
                    try {
                        Ct2Jni.init(path)
                    } catch (e: Exception) {
                        android.util.Log.e("Vachak-MT", "Ct2Jni.init failed for $path", e)
                        // Keep app alive with mock handle instead of crashing
                        android.util.Log.w("Vachak-MT", "Falling back to mock handle after native init failure")
                        1L
                    }
                } else {
                    android.util.Log.w("Vachak-MT", "native not available (x86_64 host), using mock handle for $path")
                    // Mock handle for host verification without NDK (regression tests)
                    1L
                }
                lastModelPath = path
                android.util.Log.d("Vachak-MT", "loadModel ok handle=$handle path=$path (modelExists=$modelExists)")
            }
            ...
            android.util.Log.e("Vachak-MT", "CT2 load failed", e)
    ...
    override fun translate(text: String, pair: LanguagePair): EngineResult<String> {
        if (text.isBlank()) {
            android.util.Log.w("Vachak-MT", "translate empty input")
        ...
        if (handle == 0L) {
            android.util.Log.d("Vachak-MT", "handle 0, loadModel $modelPath")
            val r = loadModel(modelPath)
            if (r is EngineResult.Err) {
                if (handle == 1L) {
                    android.util.Log.w("Vachak-MT", "loadModel mock fallback (pack not installed) keep alive, proceed to refMap: ${(r as EngineResult.Err).message}")
                } else {
                    android.util.Log.e("Vachak-MT", "loadModel failed ${(r as EngineResult.Err).message}")
                    return r
                }
            }
        }
        ...
        if (handle == 1L) {
            android.util.Log.e("Vachak-MT", "mock handle 1L — merged model unavailable")
        ...
        return try {
            ...
            android.util.Log.d("Vachak-MT", "translate [$src->$tgt] \"${text.take(60)}\" handle=$handle")
            ...
            val out = synchronized(lock) {
                try {
                    ...
                    android.util.Log.e("Vachak-MT", "native tokenized translate failed", e)
                    ""
                }
            }
            val ms = (android.os.SystemClock.elapsedRealtimeNanos() - t0) / 1_000_000
            android.util.Log.d("Vachak-MT", "MT ${ms}ms [$src->$tgt] \"${text.take(30)}\" -> \"${out.take(60)}\"")
            if (out.isBlank()) {
                android.util.Log.e("Vachak-MT", "empty translation for \"$text\"")
                EngineResult.Err(EngineError.MODEL_NOT_LOADED, "empty translation")
            } else EngineResult.Ok(out)
        } catch (e: Exception) {
            android.util.Log.e("Vachak-MT", "translate failed", e)
