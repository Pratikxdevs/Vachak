...
| `models/vits-sat.onnx` (40 MB, opset17, 22.05kHz mono, `sample_rate:22050` metadata) | Vachak (derived from Coqui VITS pattern, HiFi-GAN vocoder, Ol Chiki char tokens) — datasets above CC BY 4.0 | **Vachak-derived, CC BY 4.0 attributions preserved** (see `ml/tts/runs/santali_vits/train_config.json`) | Santali VITS via **Coqui TTS VITS** (MPL-2.0 build-time only, not shipped) + **sherpa-onnx** (Apache-2.0 runtime). Single `model.onnx` merged acoustic+vocoder, `onnx.checker` PASS, opset17. Shim export in `ml/tts/train.py` is 40 MB (20–80 budget); real Coqui fine-tune target 300–800 epochs, batch 16 (RTX 3050) / 64 (A100), 4-loss (kl + duration + mel + adv+fm). See `VOICE_CONSENT.md` + `ml/tts/dataset/santali_manifest.json`. **No Piper in APK** (Piper GPL-3.0 is training-only per `AGENTS.md` hard rule). |
| `models/tokens.txt` (55 tokens: 7 specials + 48 Ol Chiki U+1C50–U+1C7F) + `models/lexicon.txt` (66 entries, Ol Chiki char-split) | Vachak (Ol Chiki char tokenization, no espeak G2P) | **Vachak-authored** | Ol Chiki-native char tokens (`U+1C50–U+1C7F`) + `sil/eos/sp/pad/unk`. Lexicon is word→char-split phones. **No `espeak-ng-data` shipped** (`dataDir=""` at runtime) — espeak-ng has no `sat` voice (`espeak-ng-data/lang` check negative 2026-08-29); char tokens avoid 10–15 MB bloat + GPL-3.0 obligations. Verified `grep -c "ᱚ" tokens.txt >=1`. |
| `android/app/src/main/assets/vachak_models/tts/model.onnx` + `tokens.txt` + `lexicon.txt` (copied via `ml/tts/train.py`) + `android/ml/src/main/assets/vachak_models/tts/` (mirror) | Derived from `models/vits-sat.onnx` (same bytes) | Same as `models/vits-sat.onnx` | Packaged TTS slice ~40 MB mono 22.05kHz (20–80 budget). Asset extraction via `SherpaAssets.prepare(context,"tts")` recursive copy + `OfflineTts(null, config)` null AssetManager (fs path). Replaces Chinese `vits-zh-aishell3` DEV-FIXTURE (pinyin/hanzi `一 ^ i1 #0`, 39 MB) — backup note at `BACKUP_NOTE.txt`. `sherpa-onnx` AAR 1.13.0 is Apache-2.0 (vendored, no Piper). |
...
* TTS slice budget: model 40 MB + tokens 379 B + lexicon 1021 B ≈ 40 MB (20–80 green, `du -sh`); total APK ~500 MB per `AGENTS.md` budget table; no `espeak-ng-data` overhead.
